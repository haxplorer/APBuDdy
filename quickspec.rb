...
#			end
	...
	when 2
		Phase.get_phase_queue.each do |phase_item|
			...
			system("cd #{Sysvars.get_source_dir};#{phase_item.get_steps}")
		end
    ...
    def selectFile(*k)
	QFileDialog::getOpenFileName("/home","Compressed(*.tar.gz,*.tar.bz2)",self,"open file dialog","Choose a file to open")
    end
